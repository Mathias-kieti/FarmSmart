...
export default function Dashboard() {
  ...
  const [recs, setRecs] = useState<Recommendation[]>(() => getRecommendations(county));
  const top = recs[0];
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiFetch<PriceRow[]>(withQuery("/markets/prices", { county })),
    ])
      .then(([nextRecs, nextPrices]) => {
        if (!cancelled) {
          setRecs(nextRecs);
      ...
      .catch(() => {
        if (!cancelled) {
          setRecs(getRecommendations(county));
          setPrices(getMarketPrices(county));
        }
      });
  ...
  const activeGroups = groups.length
    ? groups.map((group) => ({
        ...
        farmers: group.memberIds.length,
        target: group.targetKg,
        collected: group.collectedKg,
        priceBoost: group.priceBoostPct,
        status: group.status,
      }))
    : fallbackGroups;
  ...
  const myGroups = mygroups.length
    ? mygroups.map((group) => ({
        ...
        collected: group.collectedKg,
        priceBoost: group.priceBoostPct,
        role: group.createdBy === user.id ? "Admin" : "Member",
      }))
    : fallbackMyGroups;

  const groupInsights = activeGroups.map((group) => ({
    name: group.name,
    members: group.farmers,
    avgProfit: `+${group.priceBoost}%`,
    trend: "up",
  }));
