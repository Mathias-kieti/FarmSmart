...
import type { PriceRow } from "@/types/api";
import { useEffect, useState } from "react";

export default function Markets() {
  const county = useLocationStore((s) => s.county);
  const [rows, setRows] = useState<PriceRow[]>(() => getMarketPrices(county));
  ...
  useEffect(() => {
    ...
    apiFetch<PriceRow[]>(withQuery("/markets/prices", { county }))
      ...
      .catch(() => {
        if (!cancelled) setRows(getMarketPrices(county));
      })
      .finally(() => {
  ...
  return (
    <PageShell>
      ...
      <p className="text-sm text-muted-foreground mb-6">
        {loading ? "Loading prices..." : `Live prices in ${county}, Kenya. Trends from last 7 days.`}
      </p>
