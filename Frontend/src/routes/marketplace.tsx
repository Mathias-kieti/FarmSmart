...
export default function Marketplace() {
  const { listings, loading, error, fetchListings } = useListingsStore();
  const county = useLocationStore((s) => s.county);
  const [scope, setScope] = useState<"local" | "all">("local");

  useEffect(() => {
    fetchListings({
      county: scope === "local" ? county : undefined,
      status: "Active",
    });
  }, [county, fetchListings, scope]);

  const filtered = [...listings].sort((a, b) => b.createdAt - a.createdAt);
  ...
  return (
    <PageShell>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border p-12 text-center text-destructive">
          {error}
        </div>
      ) : filtered.length === 0 ? (
